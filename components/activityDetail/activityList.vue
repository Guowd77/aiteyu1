<template>
	<view class="activityBox">
		<view class="activityBox_list" v-if="activityListDetail.length>0">
			<view class="activityBox_listItem bg-white"				
				v-for="(item,index) in activityListDetail"
				:class="index!=0?'marginTop':''"
				v-key="item.id"
				>
				<navigator :url="'/pages/activityDetail/showOneActivity?activityid='+item.id+'&isjoin='+item.is_joined">
					<view class="flexbox">
		...
		<view class="cu-load text-gray" :class="!isLoad?'loading':'over'"></view>
	</view>
</template>
