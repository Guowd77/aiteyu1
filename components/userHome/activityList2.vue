<template>
	<view class="activityBox radius shadow-warp bg-white margin-top">
		...
		<view class="activityBox_list" v-if="!hidenNumber" >
			<view class="activityBox_listItem borderBottomLine" v-if="index<activityListDetail.numlimt" v-for="(item,index) in activityListDetail.list" v-key="item.id">
				<navigator :url="'/pages/user/showOneActivity?activityid='+item.activity_id+'&isjoin='+is_joined">
					<view class="flexbox">
						<view class="itemImage flexItem">
							<image :src="item.publishactivity.post" :onerror="errorImg01"></image>
						</view>
						<view class="itemContent flexItem">
							<view class="itemTitle text-cut">{{item.publishactivity.name}}</view>
							<view class="itemTime"><image class="listIcon" src="/static/nav2/time.png"></image>{{item.publishactivity.starttime_text}}</view>
							<view class="itemTime"><image class="listIcon" src="/static/nav2/loc.png"></image>{{item.publishactivity.address}}</view>
						</view>
					</view>
				</navigator>
				<view class="itemFooter">
					...
					<text class="yibaoming">{{item.people.length}}人已报名</text>
					<button v-if="item.pass==1" >报名成功</button>
					<button v-if="item.pass==0" >报名失败</button>
					<button v-if="item.pass==null" >正在审核</button>
				</view>
			</view>
		</view>
	</view>
